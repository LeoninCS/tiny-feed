...
const avatarUrl = ref<string>(auth.avatarUrl || '')

// 本地预览：用 URL.createObjectURL 让用户选完文件立刻看到效果，
// 真正保存后才会把 avatarUrl 替换成后端返回的 /static/...。
...
async function saveAvatar() {
  ...
  if (!file) {
    toast.error('请先选择头像图片')
    return
  }
  busy.value = true
  try {
    // 用现有的 cover 上传 endpoint（jpg/png/webp 校验已对齐）。
    const res = await videoApi.uploadCover(file)
    const url = res.url || res.cover_url || ''
    if (!url) {
      toast.error('上传成功但缺少 url')
      return
    }
    // 调 updateProfile 把 URL 写进 account.avatar_url。
    await accountApi.updateProfile({ avatar_url: url })
    avatarUrl.value = url
      ...
      <div class="card">
        ...
        <div class="card" style="margin-top: 14px">
          <div class="row" style="justify-content: space-between; align-items: center">
            <p class="title" style="margin: 0">修改头像</p>
            <button class="ghost" type="button" :disabled="busy" @click="openAvatarPicker">选择图片</button>
          </div>
          <input
            ref="avatarInput"
            type="file"
            ...
            @change="pickAvatar"
          />
          <div v-if="avatarPreview" class="row" style="margin-top: 12px; gap: 12px; align-items: center">
            <UserAvatar
              :username="me.username"
              :id="me.id"
              :size="64"
            ...
            />
            <div class="row">
              <button class="primary" type="button" :disabled="busy" @click="saveAvatar">保存</button>
              <button class="ghost" type="button" :disabled="busy" @click="clearAvatarPreview">取消</button>
            </div>
          </div>
          <p v-else-if="avatarUrl" class="subtle" style="margin-top: 8px">当前已设置头像，点"选择图片"可替换</p>
          <p v-else class="subtle" style="margin-top: 8px">支持 jpg / png / webp</p>
        </div>
