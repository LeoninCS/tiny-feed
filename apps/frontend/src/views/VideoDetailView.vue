...
async function loadIsLiked() {
  ...
  try {
    ...
    // 失败时不要重置 isLiked——避免把"已点赞"状态误清掉，导致用户再点时
    // 走 else 分支又发一次 like（后端幂等不 +1，但前端 count 会乐观累加错位）。
    // 首次加载（isLiked 仍是 null）才默认 false，避免 UI 一直停在初始态。
    if (state.isLiked === null) state.isLiked = false
  }
}
...
async function toggleLike() {
  ...
  state.busy = true
  try {
    // 不在本地做"先 if else + 乐观累加 count"——前端 state.isLiked 可能因
    // loadIsLiked 失败而错乱（之前是 null/重置为 null），如果信任它选分支，
    // 用户连点 like 时会一直走 like 分支，前端 count += 1，但后端 like 是
    // 幂等的（(video_id, account_id) 唯一索引去重），所以 likes_count 实际
    // 只 +1 一次，剩下全是前端虚增。
    // 改成：先发 like/unlike，然后从后端重新拿真实 isLiked 和 video.likes_count
    // 覆盖本地状态——始终跟数据库对齐。
    if (state.isLiked) {
    ...
    const [likedRes, fresh] = await Promise.all([
      likeApi.isLiked(id.value),
      videoApi.getDetail(id.value),
    ])
    state.isLiked = likedRes.is_liked
    state.video = fresh
  } catch (e) {
...
async function publishComment() {
  ...
  drawer.error = ''
  try {
    // 后端 publish 接口返回刚插入的 *Comment 完整对象（id / username / created_at 都有），
    // 直接 push 到 drawer.comments 就能让新评论立刻出现在列表里，
    // 不用再发 listAll 重拉整个列表——发评论那一下只走一次 API。
    const newComment = await commentApi.publish(state.video.id, content)
    drawer.comments = [...drawer.comments, newComment]
    drawer.content = ''
    toast.success('评论已发布')
  } catch (e) {
    // publish 失败时不要清空 drawer.content——用户可能改完重发。
    const msg = e instanceof ApiError ? e.message : String(e)
    drawer.error = msg
    toast.error(msg)
  } finally {
        ...
        <div v-else-if="state.video" class="stage" @click="togglePlayPause">
          ...
          <div class="meta">
            <RouterLink class="author-link" :to="`/u/${state.video.author_id}`" @click.stop>
              <UserAvatar :username="state.video.username" :id="state.video.author_id" :size="34" :src="state.video.avatar_url" />
...
/* 边上弹出：drawer 占据右半区，背景视频保持原样、不被任何阴影/虚化覆盖。
   视觉区分只靠 drawer 自己的深色背景 + 左边框；不再用 box-shadow——
   阴影会往视频方向投半透明黑，让视频右边缘看起来"发暗"，被用户理解为虚化。 */
.drawer-backdrop {
  position: fixed;
  inset: 0;
